...
from models.vgg import vgg16_bn, vgg16_pretrained
import argparse
...
class WavePredictor:
    def __init__(self, model_path, device='cuda' if torch.cuda.is_available() else 'cpu'):
        self.device = device
        # 初始化模型
        self.model = vgg16_pretrained(num_classes=3).to(device)
        # 加载训练好的权重
...
def main():
    ...
    parser.add_argument('--model_path', type=str, default="./checkpoints/vgg_pre.pth", help='模型权重路径')
    parser.add_argument('--image_path', type=str, default="E:\Research__dir\Wave_level_Classfied\image_00035.jpg", help='要预测的图片路径')
